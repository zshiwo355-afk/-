...
from backend.translator.validator import ChunkRecord, JobRecord, LogEntry, SegmentRecord
...
class FileStorage:
    ...
    async def _read_json(self, path: Path, default: Any = None) -> Any:
        ...
        try:
            if not path.exists():
                ...
                return self.json_cache.get(cache_key, default)
    ...
    async def save_uploaded_file(self, file_name: str, content: bytes, job_id: str) -> Path:
        ...
        async with aiofiles.open(name_marker, "w", encoding="utf-8") as file:
